...
def build_body(version: str) -> str:
    ...
    for dmg_name, description in DOWNLOAD_VARIANTS:
        lines.append(f"- 下载文件：`{dmg_name}`（{description}）")
        lines.append(f"  Download: `{dmg_name}` ({description}).")
    ...
    lines.extend([
        ...
        "2. 打开对应 DMG，并将 Moonlight Enhanced 拖到 `Applications`。",
        "   Open the DMG and drag Moonlight Enhanced to `Applications`.",
        "3. 先正常打开一次应用。",
