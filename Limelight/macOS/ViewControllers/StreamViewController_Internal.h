...
/// Single source of truth: is this NSEvent a keyboard event?
/// Mouse / tablet / gesture / scroll events have UNDEFINED -keyCode
/// semantics. Some device drivers return garbage values that collide
/// with real kVK_ANSI_* constants (e.g. kVK_ANSI_C == 8) during
/// multi-click dispatch. ALL keyCode readers MUST gate on this FIRST.
static inline BOOL MLIsKeyboardKeyEvent(NSEvent *event) {
    if (event == nil) {
        return NO;
    }
    switch (event.type) {
...
/// (A-Z, 0-9, punctuation). Used to detect synthetic keyDown events:
/// real keyboard events for these keys ALWAYS have non-empty
/// charactersIgnoringModifiers. Synthetic events from mouse double-click
/// don't. Function keys (F1-F20), arrows, Escape etc. are NOT printable
/// and may legitimately have empty character data.
static inline BOOL MLIsPrintableANSIKeyCode(unsigned short keyCode) {
    switch (keyCode) {
...
/// ---------------------------------------------------------------------------
/// Deterministic synthetic-keyDown detector (ARCHITECTURAL FIX).
///
/// Replaces the flaky `suppressingKeyboardFromMouseEvent` time-window boolean.
///
/// Problem: Some mouse/touchpad drivers synthesize NSEventTypeKeyDown events
/// during double-click dispatch. These are FULLY FORGED events: correct type
/// (NSEventTypeKeyDown), plausible keyCode (kVK_ANSI_C == 8 is the common
/// collision value), non-empty charactersIgnoringModifiers, even plausible
/// modifierFlags.  Time-window suppression fails because:
///   - the forged event may arrive >300ms after the last mouseUp (AppKit busy),
...
///       charactersIgnoringModifiers.length == 1, and the character at
///       position 0 is the expected lowercase ANSI glyph for that keyCode in
///       the standard US-104 layout.  Synthetic "C" events are emitted with
///       garbage characters (or a correct glyph but wrong case) depending on
///       the current keyboard layout; the real event always matches.
///
/// 3. MODIFIER CONSISTENCY CHECK — If NSEventModifierFlagNumericPad is set
///    while the keyCode is NOT a keypad keyCode, or NSEventModifierFlagHelp
///    is set for a non-Help keyCode, etc. these are obvious synthetic-tell.
///
/// In addition, StreamViewController tracks a "last mouse button event
/// monotonic timestamp" that is used by the proximity check.  It is updated
/// from EVERY mouse button handler.
/// ---------------------------------------------------------------------------

/// Monotonic milliseconds (mach_absolute_time based, never wall-clock drift).
static inline uint64_t MLMonotonicMillis(void) {
    ...
    if (_tb.denom != 0) {
        nsec = nsec * _tb.numer / _tb.denom;
    }
    return nsec / 1000000ULL;
}

/// True if the given printable-ANSI keyCode matches the given character
/// for the standard US keyboard layout (lowercase unmodified).  Synthetic
/// double-click "C" events frequently fail this check because they carry
/// the layout-resolved character of whatever the current text input source
/// would insert — NOT the actual ANSI glyph for the keyCode.
static inline BOOL MLPrintableANSIKeyCodeMatchesCharacter(unsigned short keyCode, unichar c) {
    // Build mapping for A-Z, 0-9, common punctuation (kVK_ANSI_* constants).
    switch (keyCode) {
        case kVK_ANSI_A: return c == 'a';
        case kVK_ANSI_B: return c == 'b';
        case kVK_ANSI_C: return c == 'c';
        case kVK_ANSI_D: return c == 'd';
        case kVK_ANSI_E: return c == 'e';
        case kVK_ANSI_F: return c == 'f';
        case kVK_ANSI_G: return c == 'g';
        case kVK_ANSI_H: return c == 'h';
        case kVK_ANSI_I: return c == 'i';
        case kVK_ANSI_J: return c == 'j';
        case kVK_ANSI_K: return c == 'k';
        case kVK_ANSI_L: return c == 'l';
        case kVK_ANSI_M: return c == 'm';
        case kVK_ANSI_N: return c == 'n';
        case kVK_ANSI_O: return c == 'o';
        case kVK_ANSI_P: return c == 'p';
        case kVK_ANSI_Q: return c == 'q';
        case kVK_ANSI_R: return c == 'r';
        case kVK_ANSI_S: return c == 's';
        case kVK_ANSI_T: return c == 't';
        case kVK_ANSI_U: return c == 'u';
        case kVK_ANSI_V: return c == 'v';
        case kVK_ANSI_W: return c == 'w';
        case kVK_ANSI_X: return c == 'x';
        case kVK_ANSI_Y: return c == 'y';
        case kVK_ANSI_Z: return c == 'z';
        case kVK_ANSI_0: return c == '0';
        case kVK_ANSI_1: return c == '1';
        case kVK_ANSI_2: return c == '2';
        case kVK_ANSI_3: return c == '3';
        case kVK_ANSI_4: return c == '4';
        case kVK_ANSI_5: return c == '5';
        case kVK_ANSI_6: return c == '6';
        case kVK_ANSI_7: return c == '7';
        case kVK_ANSI_8: return c == '8';
        case kVK_ANSI_9: return c == '9';
        case kVK_ANSI_Equal: return c == '=';
        case kVK_ANSI_Minus: return c == '-';
        case kVK_ANSI_RightBracket: return c == ']';
        case kVK_ANSI_LeftBracket: return c == '[';
        case kVK_ANSI_Quote: return c == '\'';
        case kVK_ANSI_Semicolon: return c == ';';
        case kVK_ANSI_Backslash: return c == '\\';
        case kVK_ANSI_Comma: return c == ',';
        case kVK_ANSI_Slash: return c == '/';
        case kVK_ANSI_Period: return c == '.';
        case kVK_ANSI_Grave: return c == '`';
        default: return NO;
    }
}
...
///                           Pass 0 if none.
/// @param outReason          if non-NULL and the event is synthetic, a
///                           short static string explaining which check fired.
static inline BOOL MLKeyDownIsSyntheticDoubleClick(NSEvent *event,
                                                   uint64_t lastMouseButtonAtMs,
                                                   const char **outReason) {
    if (event == nil || event.type != NSEventTypeKeyDown) {
        if (outReason != NULL) *outReason = "not-keyDown";
        return NO;
    }
    ...
    if (MLIsPrintableANSIKeyCode(kc)) {
        NSString *chars = event.charactersIgnoringModifiers;
        if (chars.length != 1) {
            if (outReason != NULL) *outReason = "printable-bad-chars-length";
            return YES;
        }
        unichar c = [chars characterAtIndex:0];
        if (!MLPrintableANSIKeyCodeMatchesCharacter(kc, (unichar)tolower((int)c))) {
            if (outReason != NULL) *outReason = "printable-char-mismatch";
            return YES;
        }
    }

    // ---------- 3. SPURIOUS MODIFIER FLAGS ----------
    ...
    if ((mods & NSEventModifierFlagNumericPad) &&
        ...
        if (outReason != NULL) *outReason = "spurious-numpad-mod";
        return YES;
    }
...
/// --- LEGACY (deprecated, replaced by deterministic detector) ---
/// Kept for binary/header compatibility. WRITES to these are now no-ops;
/// reads always return 0/NO. The new architecture uses
/// MLKeyDownIsSyntheticDoubleClick() + lastMouseButtonEventAtMs below.
@property (nonatomic) BOOL suppressingKeyboardFromMouseEvent;
@property (nonatomic) NSUInteger keyboardSuppressionClearToken;
/// --- NEW ARCHITECTURE ---
/// Monotonic-millis timestamp of the most recent left/right/other mouse
/// button DOWN or UP event processed by ANY mouse button handler.
/// Used by the proximity check in MLKeyDownIsSyntheticDoubleClick().
/// 0 means no mouse event yet this stream.
@property (nonatomic) uint64_t lastMouseButtonEventAtMs;
